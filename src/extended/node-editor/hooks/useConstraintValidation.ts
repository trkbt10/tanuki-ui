...
export const useConstraintValidation = (): ConstraintValidationHook => {
  ...
  const validateNodeOperation = React.useCallback((
    ...
    const result = validateNodeConstraints(
      node,
      nodeDefinition,
      allNodes,
      allConnections,
  ...
  const validateConnectionOperation = React.useCallback((
    ...
    const allConstraints = [
      ...(fromNodeDefinition.constraints || []),
      ...(toNodeDefinition.constraints || []),
    ];
  ...
  const canPerformAction = React.useCallback((
    ...
    switch (action.type) {
      case "ADD_NODE": {
        ...
        const result = validateNodeOperation(
          tempNode,
          "create",
          currentState.nodes,
      ...
      case "ADD_CONNECTION": {
        ...
        const result = validateConnectionOperation(
          fromNode,
          toNode,
          "connect",
          currentState.nodes,
