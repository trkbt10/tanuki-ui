...
export const Toolbar: React.FC<ToolbarProps> = ({ className, floating = false, position = "top" }) => {
  ...
  const commonNodeTypes = React.useMemo(() => {
    return nodeDefinitions.filter((def) => ["standard", "input", "output", "process", "group"].includes(def.type)).slice(0, 5); // Limit to 5 most common types
  }, [nodeDefinitions]);
  ...
  return <div className={`${styles.topToolbar} ${className || ""}`}>{toolbarContent}</div>;
};
