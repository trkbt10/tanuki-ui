...
export const InspectorPanel: React.FC<InspectorPanelProps> = ({ 
  ...
  const selectedNode = actionState.selectedNodeIds.length > 0 
    ? nodeEditorState.nodes[actionState.selectedNodeIds[0]] 
    : null;
  ...
  const selectedConnection = actionState.selectedConnectionIds.length > 0 
    ? nodeEditorState.connections[actionState.selectedConnectionIds[0]] 
    : null;

  // Handle alignment operations
  const handleAlignNodes = React.useCallback((alignmentType: string, nodes: typeof selectedNodes) => {
              ...
              <div className={styles.inspectorField}>
                <label>
                  <input
                    ...
                    onChange={(e) => {
                      canvasDispatch(canvasActions.updateGridSettings({
